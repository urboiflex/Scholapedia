...
namespace WAPPSS
{
    public partial class StudentAnnouncements : System.Web.UI.Page
    {
        ...
        private bool IsUserStudent()
        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(connStr))
                {
                    ...
                    return category != null && category.ToString().Equals("Student", StringComparison.OrdinalIgnoreCase);
                }
            }
            catch
            {
        ...
        private void LoadStudentAnnouncements()
        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(connStr))
                {
                    ...
                    // Query announcements for students only and all users
                    var cmd = new SqlCommand(@"
                        SELECT AnnouncementID, adminID, title, content, target, time
                        FROM Announcements
                        WHERE target IN ('Students only', 'Students', 'All users', 'All Users')
        ...
        private void CreateAnnouncementCard(SqlDataReader reader)
        {
            ...
            if (target.Contains("Students"))
            {
                targetBadgeClass = "target-students";
                targetText = "Students Only";
            }
