<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TestEdit.aspx.cs" Inherits="WAPPSS.TestEdit" %>
<!DOCTYPE html>
<html lang="en">
<head runat="server">
  <meta charset="UTF-8" />
  <title>Edit Test - Scholapedia</title>
...
<body id="bodyTag" runat="server">
    ...
    <div class="test-container">
      <asp:Panel ID="pnlSuccess" runat="server" Visible="false" CssClass="success-message">
        ...
        <div style="margin-top:16px;">
          <!-- ENSURE course is always populated in link -->
          <a href='course_detail.aspx?course=<%= GetCourseForUrl() %>' 
      ...
      <asp:Panel ID="pnlTestForm" runat="server" CssClass="test-form">
        <div class="test-header">Edit Test</div>
        ...
        <asp:Button ID="btnSaveTest" runat="server" Text="Update Test" CssClass="submit-test-btn" OnClick="btnSaveTest_Click" />
      </asp:Panel>
  ...
  <script>
      let questions = [];
    <% if (!string.IsNullOrEmpty((string)Page.Items["QuestionsJSON"])) { %>
      questions = <%= Page.Items["QuestionsJSON"] %>;
    <% } %>
    function renderQuestions() {
      ...
      <div class="question-block" data-index="${i}">
        ...
        <input type="text" class="q-text" value="${q.text ? q.text.replace(/"/g, '&quot;') : ''}" placeholder="Enter question..." maxlength="1000" required/>
        ...
        <div class="options-section" style="${q.type === 'radio' || q.type === 'checkbox' ? '' : 'display:none;'};margin-top:14px;">
          ${(q.options || []).map((opt, j) => `
            <div class="option-row" data-oid="${j}">
              <input type="text" class="option-text" value="${opt.text ? opt.text.replace(/"/g, '&quot;') : ''}" placeholder="Option..." maxlength="500" required/>
    ...
    window.onload = function() { renderQuestions(); };
  </script>
  <script>
