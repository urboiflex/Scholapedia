<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="testForStudents.aspx.cs" Inherits="WAPPSS.testForStudents" %>
<!DOCTYPE html>
<html lang="en">
<head runat="server">
  <meta charset="UTF-8" />
  <title>Student Test - Scholapedia</title>
...
<body id="bodyTag" runat="server">
  <form id="form1" runat="server">
    <div class="preview-container">
      ...
      <asp:Panel ID="QuestionsPanel" runat="server"></asp:Panel>
      ...
      <a href='StudentCourseDetails.aspx?id=<%= CourseID %>' 
  ...
  <script>
      ...
      let testID = document.getElementById('<%= hfTestID.ClientID %>').value;
      let userID = document.getElementById('<%= hfUserID.ClientID %>').value;
          ...
          // Save to database
          saveTestResult(userID, testID, score, function (success, message) {
              let res = `<div style="font-size:1.23rem;margin-bottom:12px;">Test Completed!</div>`;
              if (total > 0) res += `<div style="font-size:1.08rem;margin-bottom:10px;">Score: <b>${score}/${total}</b></div>`;
              if (success) {
                  res += `<div style="color:#13b113;margin-bottom:10px;">✓ Results saved successfully!</div>`;
              } else {
                  res += `<div style="color:#d80000;margin-bottom:10px;">⚠ ${message}</div>`;
              }
              res += html;
      ...
      function saveTestResult(userID, testID, score, callback) {
          var xhr = new XMLHttpRequest();
          xhr.open('POST', 'testForStudents.aspx/SaveTestResult', true);
              ...
              if (xhr.readyState === 4) {
                  if (xhr.status === 200) {
                      try {
                          ...
                          callback(response.d.success, response.d.message);
                      } catch (e) {
                          callback(false, 'Error processing response');
                      }
                  } else {
                      callback(false, 'Error saving results');
                  }
              }
          };
